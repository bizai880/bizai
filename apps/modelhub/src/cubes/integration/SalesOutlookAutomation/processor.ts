...
export class SalesOutlookAutomationCube extends BaseCube {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          emailConfig: {
            ...
            required: ['senderEmail', 'smtpServer', 'smtpUsername', 'smtpPassword']
          },
          recipients: {
            type: 'object',
            properties: {
  ...
  async process(input: any): Promise<CubeExecutionResult> {
    try {
      ...
      // 2. تهيئة محول البريد
      this.transporter = await this.initializeEmailTransporter(this.config.emailConfig);
      ...
      return {
        ...
        results,
        summary,
        nextCheck: this.calculateNextCheck(input.options?.checkInterval || 'daily'),
        executionTime
      };
  ...
  private async initializeEmailTransporter(config: EmailConfig): Promise<nodemailer.Transporter> {
  ...
  private createAlertRules(recipients: any, options: any): any[] {
    ...
    rules.push({
      ...
      condition: (record: SalesRecord) => {
        ...
        while (currentDate < now) {
          ...
          if (options?.workingDays?.includes(dayOfWeek) || 
              (!options?.workingDays && dayOfWeek !== 5 && dayOfWeek !== 6)) { // الجمعة والسبت
            workingDaysDiff++;
          }
      ...
      recipients: [{
        ...
        email: record => this.extractEmail(record.responsiblePerson) || recipients.salesAdmin,
    ...
    rules.push({
      ...
      recipients: [
        { name: 'Sales Admin', email: recipients.salesAdmin, role: 'Sales Admin' },
        { name: 'Sales Engineer', email: recipients.salesEngineer, role: 'Sales Engineer' },
        { name: 'Manager', email: recipients.manager, role: 'Manager' }
      ],
      subjectTemplate: 'Delivery Delay Alert – Order {orderNumber || rfqNumber}',
    ...
    rules.push({
      ...
      subjectTemplate: 'High Priority Sales Order – Immediate Attention Required',
    ...
    rules.push({
      ...
      recipients: [
        { name: 'Sales Manager', email: recipients.manager, role: 'Sales Manager' }
      ],
  ...
  private async processSalesRecords(records: SalesRecord[], rules: any[]): Promise<AlertResult[]> {
    ...
    for (const record of records) {
      for (const rule of rules) {
        try {
          ...
          if (lastTriggered) {
            const hoursSinceLast = (Date.now() - lastTriggered.getTime()) / (1000 * 60 * 60);
          ...
          const emailSubject = this.generateEmailSubject(rule.subjectTemplate, record);
