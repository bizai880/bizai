...
export async function scheduleAutomation(
  ...
  if (!cronExpression) {
    console.log(`ℹ️ Manual execution only, no scheduling for interval: ${interval}`);
    return;
  }
  
  console.log(`⏰ Scheduling automation to run: ${interval} (${cronExpression})`);
  
  schedule(cronExpression, async () => {
    try {
      console.log(`🚀 Running scheduled automation at ${new Date().toISOString()}`);
...
export function stopScheduler(): void {
  ...
  console.log('⏹️ Scheduler stopped (implementation depends on your scheduling library)');
}