...
export async function parseExcelFile(
  ...
  try {
    ...
    const records: SalesRecord[] = data.map((row: any, index: number) => {
      ...
      return {
        customerName: findColumn(['Customer Name', 'اسم العميل', 'العميل', 'Customer']),
        rfqNumber: findColumn(['RFQ Number', 'رقم RFQ', 'RFQ', 'رقم الطلب']),
        quotationStatus: findColumn(['Quotation Status', 'حالة العرض', 'حالة الاقتباس']),
        deliveryStatus: findColumn(['Delivery Status', 'حالة التسليم', 'الحالة']),
        expectedDeliveryDate: parseDate(findColumn([
          'Expected Delivery Date', 
          'موعد التسليم المتوقع',
          'تاريخ التسليم'
        ])),
        priority: mapPriority(findColumn(['Priority', 'الأولوية', 'الأهمية'])),
        escalationFlag: mapEscalationFlag(findColumn([
          'Escalation Flag', 
          'علم التصعيد', 
          'تصعيد'
        ])),
        ...
        lastFollowUpDate: parseDate(findColumn([
          'Last Follow-Up Date',
          'تاريخ آخر متابعة',
          'آخر متابعة'
        ])),
        orderNumber: findColumn(['Order Number', 'رقم الأمر', 'رقم الطلب النهائي']),
        escalationReason: findColumn(['Escalation Reason', 'سبب التصعيد']),
        currentStatus: findColumn(['Current Status', 'الحالة الحالية']),
        recommendedAction: findColumn(['Recommended Action', 'الإجراء المقترح'])
      };
    });
...
function mapPriority(priorityValue: any): 'Low' | 'Medium' | 'High' | 'Critical' {
  if (!priorityValue) return 'Medium';
  
  const value = priorityValue.toString().toLowerCase();
  
  if (value.includes('critical') || value.includes('حرج') || value.includes('أقصى')) {
    return 'Critical';
  }
  
  if (value.includes('high') || value.includes('عالي') || value.includes('مرتفع')) {
    return 'High';
  }
  
  if (value.includes('low') || value.includes('منخفض') || value.includes('واطي')) {
    return 'Low';
  }
