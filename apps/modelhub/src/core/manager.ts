...
import { CubeMetadata, CubeExecutionInput, CubeExecutionResult } from '@bizai/shared';
...
export class CubeManager {
  ...
  async registerCube(cube: BaseCube): Promise<void> {
    ...
    await this.redis.hset('cubes:registry', cube.metadata.id, JSON.stringify(cube.metadata));
    
    console.log(`✅ Cube registered: ${cube.metadata.name} (${cube.metadata.id})`);
  }
