...
export class AICore {
  ...
  async process(request: AIRequest): Promise<AIResponse> {
    ...
    for (const modelItem of fallbackModels) {
      try {
        ...
        const provider = this.providers[modelItem.provider as keyof typeof this.providers]
        ...
        console.warn(`${modelItem.provider} failed:`, error?.message || 'Unknown error')
        lastError = error
  ...
  async processStream(request: AIRequest) {
    const model = this.selectModel(request)
    const provider = this.providers[model.provider as keyof typeof this.providers]
  ...
  async analyzeExcelRequest(description: string): Promise<any> {
    ...
    try {
      ...
      console.error('Failed to parse AI analysis:', error?.message || 'Unknown error')
  ...
  async generateDashboard(analysis: any): Promise<any> {
    ...
    try {
      ...
      console.error('Failed to parse dashboard config:', error?.message || 'Unknown error')
  ...
  private selectModel(request: AIRequest): AIModel {
    ...
    const isArabic = request.language === 'ar' || (request.prompt && request.prompt.match(/[\u0600-\u06FF]/))
    ...
    return filteredModels.sort((a, b) => a.fallbackOrder - b.fallbackOrder)[0] || this.models[0]
  }
  ...
  private async logUsage(response: AIResponse, request: AIRequest): Promise<void> {
    try {
      ...
      console.error('Failed to log AI usage:', error?.message || 'Unknown error')
    }
  }
  ...
  private parseStructuredResponse(text: string): any {
    ...
    for (const line of lines) {
      if (line.includes('أعمدة') || line.includes('columns') || line.includes('column')) {
        ...
        const chartType = line.includes('شريطي') || line.includes('bar') ? 'bar' :
                        line.includes('دائري') || line.includes('pie') ? 'pie' :
                        line.includes('خط') || line.includes('line') ? 'line' : 'bar'
  ...
  private guessColumnType(text: string): string {
    ...
    if (lower.includes('تاريخ') || lower.includes('date') || lower.includes('time')) return 'date'
    if (lower.includes('رقم') || lower.includes('عدد') || lower.includes('number') || lower.includes('قيمة') || lower.includes('value') || lower.includes('كمية') || lower.includes('amount')) return 'number'
    if (lower.includes('نعم') || lower.includes('لا') || lower.includes('yes') || lower.includes('no') || lower.includes('صحيح') || lower.includes('خاطئ')) return 'boolean'
    if (lower.includes('صورة') || lower.includes('image') || lower.includes('رابط') || lower.includes('link') || lower.includes('url')) return 'url'
  ...
  private getDefaultTemplate(description: string): any {
    ...
    return {
      ...
      columns: [
        { name: isArabic ? 'المعرف' : 'ID', type: 'text', validation: 'unique' },
        { name: isArabic ? 'الاسم' : 'Name', type: 'text' },
        { name: isArabic ? 'القيمة' : 'Value', type: 'number', formula: 'SUM' },
        { name: isArabic ? 'التاريخ' : 'Date', type: 'date' },
        { name: isArabic ? 'الحالة' : 'Status', type: 'text', validation: 'list:Active,Inactive,Pending' }
      ],
      charts: [
        { type: 'bar', title: isArabic ? 'توزيع القيم' : 'Value Distribution', data_range: 'C2:C100' },
        { type: 'pie', title: isArabic ? 'توزيع الحالات' : 'Status Distribution', data_range: 'E2:E100' }
      ],
      metadata: { 
        is_default: true, 
  ...
  private getDefaultDashboardConfig(): any {
    return {
      ...
      widgets: [
        { type: 'statistic', position: [0, 0], size: [2, 2], title: 'المؤشرات الرئيسية' },
        { type: 'chart', position: [0, 2], size: [2, 4], title: 'التحليل البياني' },
        { type: 'table', position: [2, 0], size: [2, 6], title: 'البيانات التفصيلية' }
      ],
      theme: {
        primaryColor: '#3b82f6',
