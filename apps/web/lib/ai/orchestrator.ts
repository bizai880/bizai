import { AIRequest, AIResponse, AIProvider, AIModelConfig, CubeMetadata, CubeExecutionInput, CubeExecutionResult } from '@bizai/shared';
...
export interface SystemRequirements {
  ...
  category: 'hr' | 'sales' | 'inventory' | 'education' | 'healthcare' | 'general';
  features: string[];
  cubes: {
    required: string[];
    recommended: string[];
...
export class AIOrchestrator {
  ...
  private fallbackOrder: AIProvider[] = ['groq', 'gemini', 'huggingface', 'local'];
  ...
  async orchestrateCubes(request: CubeOrchestrationRequest): Promise<CubeOrchestrationResult> {
    ...
    try {
      ...
      const systemRequirements = await this.analyzeSystemRequirements(request.description);
      
      // 2. تحديد المكعبات المطلوبة
      const requiredCubes = request.cubes || await this.selectCubesForSystem(systemRequirements);
      
      // 3. تنفيذ المكعبات بالتسلسل
      const executionOrder = this.determineExecutionOrder(requiredCubes, systemRequirements);
      ...
      for (const cubeId of executionOrder) {
        ...
        const cubeInput = await this.prepareCubeInput(cubeId, request.description, results);
        
        // 5. تنفيذ المكعب
        const cubeResult = await this.executeCube(cubeId, cubeInput, request.options);
      ...
      const system = await this.buildSystemFromResults(systemRequirements, results);
      
      // 8. اقتراح تحسينات
      const suggestions = await this.generateSuggestions(systemRequirements, results);
  ...
  private async analyzeSystemRequirements(description: string): Promise<SystemRequirements> {
  ...
  private guessSystemRequirements(description: string): SystemRequirements {
    ...
    if (lowerDesc.includes('تتبع') || lowerDesc.includes('tracking')) type = 'tracking';
    if (lowerDesc.includes('داشبورد') || lowerDesc.includes('dashboard')) type = 'dashboard';
    if (lowerDesc.includes('تحليل') || lowerDesc.includes('analytics')) type = 'analytics';
    if (lowerDesc.includes('إدارة') || lowerDesc.includes('management')) type = 'management';
    
    // تحديد المجال
    let category: SystemRequirements['category'] = 'general';
    if (lowerDesc.includes('موظف') || lowerDesc.includes('hr')) category = 'hr';
    if (lowerDesc.includes('مبيعات') || lowerDesc.includes('sales')) category = 'sales';
    if (lowerDesc.includes('مخزون') || lowerDesc.includes('inventory')) category = 'inventory';
    if (lowerDesc.includes('تعليم') || lowerDesc.includes('education')) category = 'education';
    if (lowerDesc.includes('صحة') || lowerDesc.includes('health')) category = 'healthcare';
    
    // تحديد الميزات
    const features: string[] = [];
    if (lowerDesc.includes('وجه') || lowerDesc.includes('face')) features.push('face_recognition');
    if (lowerDesc.includes('مشاعر') || lowerDesc.includes('sentiment')) features.push('sentiment_analysis');
    if (lowerDesc.includes('تنبؤ') || lowerDesc.includes('forecast')) features.push('forecasting');
    if (lowerDesc.includes('إشعار') || lowerDesc.includes('notification')) features.push('notifications');
  ...
  private async selectCubesForSystem(requirements: SystemRequirements): Promise<string[]> {
  ...
  private determineExecutionOrder(cubes: string[], requirements: SystemRequirements): string[] {
  ...
  private async prepareCubeInput(cubeId: string, description: string, previousResults: CubeExecutionResult[]): Promise<any> {
    const cubeMetadata = this.cubeRegistry.get(cubeId);
    if (!cubeMetadata) {
    ...
    switch (cubeId) {
      ...
      case 'sentiment_analysis':
        const textData = await this.extractTextData(description, previousResults);
        return {
          text: textData,
          language: 'ar'
        };
  ...
  private async executeCube(cubeId: string, input: any, options?: any): Promise<CubeExecutionResult> {
    const startTime = Date.now();
  ...
  private async executeCubeLocally(cubeId: string, input: any): Promise<CubeExecutionResult> {
    const startTime = Date.now();
  ...
  private async executeSentimentAnalysis(input: any): Promise<any> {
    ...
    if (huggingface) {
      try {
        const response = await huggingface.sentimentAnalysis(input.text, input.language);
        return response;
      } catch (error) {
  ...
  private async executeDataForecasting(input: any): Promise<any> {
    ...
    if (data.length === 0) {
      ...
      const demoData = Array.from({ length: 30 }, (_, i) => 
        Math.random() * 100 + 50 + Math.sin(i * 0.2) * 20
      );
  ...
  private forecastLinear(data: number[], periods: number): any {
    ...
    const accuracy = Math.max(0.7, 1 - Math.abs(predictions[0] - lastValue) / lastValue);
  ...
  private async buildSystemFromResults(requirements: SystemRequirements, cubeResults: CubeExecutionResult[]): Promise<any> {
    ...
    return {
      ...
      estimatedDevelopmentTime: this.calculateDevelopmentTime(requirements, cubeResults)
    };
  }

  private async generateSuggestions(requirements: SystemRequirements, cubeResults: CubeExecutionResult[]): Promise<string[]> {
  ...
  private async extractTextData(description: string, previousResults: CubeExecutionResult[]): Promise<string> {
  ...
  private async prepareExcelData(cubeResults: CubeExecutionResult[]): Promise<any[]> {
    const data: any[] = [];
    ...
    return data.length > 0 ? data : [
      { example: 'بيانات تجريبية', value: 100, date: new Date().toISOString() }
    ];
  }
  ...
  private calculateDevelopmentTime(requirements: SystemRequirements, cubeResults: CubeExecutionResult[]): number {
  ...
  private extractCubesFromText(text: string): string[] {
    ...
    if (lowerText.includes('وجه') || lowerText.includes('face')) cubes.push('face_recognition');
    if (lowerText.includes('مشاعر') || lowerText.includes('sentiment')) cubes.push('sentiment_analysis');
    if (lowerText.includes('تنبؤ') || lowerText.includes('forecast')) cubes.push('data_forecasting');
