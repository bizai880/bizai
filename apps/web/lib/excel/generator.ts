...
export class ExcelGenerator {
  ...
  private addData(worksheet: any, data: any[], columns: ExcelColumn[]) {
    data.forEach((rowData, rowIndex) => {
      const row = worksheet.addRow(
        columns.map(col => {
          const value = rowData[col.name] || rowData[col.name.toLowerCase()] || '';
  ...
  private createSummarySheet(workbook: any, config: ExcelWorkbook) {
    ...
    summarySheet.addRow(['إجمالي الأعمدة:', config.sheets.reduce((acc, sheet) => acc + sheet.columns.length, 0)]);
    summarySheet.addRow(['إجمالي الصفوف:', config.sheets.reduce((acc, sheet) => acc + sheet.data.length, 0)]);
    summarySheet.addRow(['']);
 ...
 */
export async function generateExcelInBrowser(config: ExcelWorkbook, filename: string = 'bizai-export.xlsx'): Promise<void> {
  ...
  try {
    ...
    const worksheet = workbook.addWorksheet(config.sheets[0]?.name || 'البيانات');
    ...
    if (config.sheets[0]) {
      ...
      sheet.data.forEach(rowData => {
        worksheet.addRow(
          sheet.columns.map(col => rowData[col.name] || '')
        );
      });
    }
