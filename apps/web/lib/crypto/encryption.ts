...
export class EncryptionService {
	...
	decrypt(encryptedText: string): string {
		...
		const tag = Buffer.from(
			encryptedText.substring(encryptedText.length - TAG_LENGTH * 2),
			"hex",
		);
		const encrypted = encryptedText.substring(
			IV_LENGTH * 2,
			encryptedText.length - TAG_LENGTH * 2,
		);
...
export class JWTService {
	...
	constructor() {
		this.jwtSecret =
			process.env.JWT_SECRET || EncryptionService.generateKey(64);
	}
	...
	private parseExpiresIn(expiresIn: string): number {
		...
		switch (unit) {
			...
			default:
				return 7 * 24 * 60 * 60; // 7 أيام افتراضياً
		}
	}
