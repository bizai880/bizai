...
export const serveInngest = serve({
	client: inngest,
	functions: [generateExcel, generateDashboard, sendNotification, logActivity],
	// إعدادات الإرسال
	streaming: true,
...
export async function sendGenerateRequest(
	data: Events["bizai/generate.request"]["data"],
) {
	return sendEvent("bizai/generate.request", data);
}

export async function sendGenerateCompleted(
	data: Events["bizai/generate.completed"]["data"],
) {
	return sendEvent("bizai/generate.completed", data);
}

export async function sendGenerateProgress(
	data: Events["bizai/generate.progress"]["data"],
) {
