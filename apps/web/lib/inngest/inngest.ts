...
export class InngestClient {
	...
	async send(
		event: InngestEvent,
	): Promise<{ success: boolean; functionId?: string; result?: any }> {
		console.log(`Inngest event received: ${event.name}`, event.data);

		// Find matching functions
		const matchingFunctions = Array.from(this.functions.values()).filter(
			(f) => f.trigger.event === event.name,
		);
