...
export function CubeProperties({ cube, onUpdate }: CubePropertiesProps) {
  ...
  const handleSave = () => {
    ...
    const updatedCube: CubeDefinition = {
      ...
      tags,
      inputSchema: cube?.inputSchema || { type: 'object', properties: {}, required: [] },
  ...
  const categoryOptions = [
    { value: 'vision', label: '👁️ الرؤية الحاسوبية', description: 'معالجة الصور والفيديو' },
    { value: 'nlp', label: '💬 معالجة اللغة', description: 'تحليل النصوص واللغات' },
    { value: 'data', label: '📊 تحليل البيانات', description: 'معالجة وتحليل البيانات' },
    { value: 'integration', label: '🔗 التكامل', description: 'تكامل مع أنظمة خارجية' },
    { value: 'custom', label: '🎨 مخصص', description: 'مكعب مخصص حسب الاحتياج' }
  ];
  ...
  const getPerformanceLevel = (time: number) => {
    return performanceLevels.find(level => time <= level.time) || performanceLevels[3];
  };
      ...
      <div className="space-y-4 pt-4 border-t">
        ...
        <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div>
            ...
            <div className="text-sm text-gray-500">للمكعبات الثقيلة التي تحتاج معالجة متقدمة</div>
          </div>
          <Switch
            checked={requiresGPU}
            onCheckedChange={setRequiresGPU}
          />
        </div>
        ...
        <div className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
          <div>
            ...
            <div className="text-sm text-gray-500">سيظهر في المكتبة ويكون متاحاً للاستخدام</div>
          </div>
          <Switch
            checked={isActive}
