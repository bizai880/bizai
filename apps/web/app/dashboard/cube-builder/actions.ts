...
export async function updateCube(
	cubeId: string,
	updates: Partial<CubeDefinition>,
): Promise<CubeOperationResult> {
	try {
		const cookieStore = await cookies();
		const token = cookieStore.get("auth-token")?.value;
		...
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/cubes/${cubeId}`,
			{
				method: "PATCH",
				headers: {
					"Content-Type": "application/json",
...
// اختبار مكعب
export async function testCube(
	cubeId: string,
	inputData: any,
	...
	try {
		...
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/cubes/${cubeId}/test`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
...
export async function deployCube(cubeId: string): Promise<CubeOperationResult> {
	try {
		...
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/cubes/${cubeId}/deploy`,
			{
				method: "POST",
				headers: {
					Authorization: `Bearer ${token}`,
...
export async function importCube(file: File): Promise<CubeOperationResult> {
	try {
		...
		const cube: CubeDefinition = {
			...
			tags: cubeData.tags || [],
			inputSchema: cubeData.inputSchema || {
				type: "object",
				properties: {},
				required: [],
			},
...
// توليد كود المكعب
export async function generateCubeCode(
	cube: CubeDefinition,
	...
	try {
		...
		const response = await fetch(
			`${process.env.NEXT_PUBLIC_API_URL}/cubes/generate-code`,
			{
				method: "POST",
				headers: {
					"Content-Type": "application/json",
...
// نسخ مكعب
export async function duplicateCube(
	cubeId: string,
	newName: string,
): Promise<CubeOperationResult> {
	try {
