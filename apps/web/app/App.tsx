...
export default function App({ children }: { children: React.ReactNode }) {
  ...
  const minimalPages = ['/auth/login', '/auth/register', '/auth/forgot-password']
  const isMinimalPage = minimalPages.some(page => pathname?.startsWith(page))

  if (isLoading) {
    return (
      <html lang="ar" dir="rtl">
        <body style={{ margin: 0, padding: 0, fontFamily: "'IBM Plex Sans Arabic', 'Inter', sans-serif" }}>
          <div style={{ 
            minHeight: '100vh', 
            display: 'flex', 
            alignItems: 'center', 
            ...
            <LoadingProvider>
              <AuthProvider>
                <NotificationProvider>
                  ...
                  <main style={isMinimalPage ? {} : { paddingTop: '4rem', minHeight: '100vh' }}>
                    {children}
                  </main>
