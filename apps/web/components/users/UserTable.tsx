...
export default function UserTable({
  ...
  const getRoleColor = (role: string) => {
    switch (role) {
      ...
      case 'moderator': return 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20'
          ...
          <tbody>
            {users.map((user, index) => (
              ...
              >
                ...
                <td className="py-4 px-6">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getSubscriptionColor(user.subscription)}`}>
                    {user.subscription === 'enterprise' ? 'مؤسسة' : 
                     user.subscription === 'pro' ? 'احترافي' : 'مجاني'}
                  </span>
                </td>
