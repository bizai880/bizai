...
export default function DashboardCard({
	...
	const getStats = () => {
		switch (type) {
			case "dashboard":
				return { uses: "1.2K", rating: "4.8" };
			case "tracking":
				return { uses: "890", rating: "4.6" };
			default:
				return { uses: "540", rating: "4.9" };
		}
	};
	...
	return (
		<Suspense fallback={<CardSkeleton />}>
			...
			>
				{/* Header */}
				<div
					className={`h-2 ${color.includes("from-") ? `bg-gradient-to-r ${color}` : color}`}
				/>

				<div className="p-6">
					<div className="flex items-start justify-between mb-4">
						<div
							className={`w-12 h-12 rounded-xl ${color.includes("from-") ? `bg-gradient-to-br ${color}` : `bg-${color}`} p-3 text-white`}
						>
							{getIcon()}
						</div>
