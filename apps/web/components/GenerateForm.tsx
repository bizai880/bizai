...
import { Send, FileSpreadsheet, LayoutDashboard, Users, Settings, Globe } from 'lucide-react';

const templateTypes = [
  { id: 'excel', label: 'ملف Excel', icon: <FileSpreadsheet className="w-5 h-5" />, color: 'from-green-500 to-emerald-400' },
  { id: 'dashboard', label: 'داشبورد', icon: <LayoutDashboard className="w-5 h-5" />, color: 'from-primary to-primary-light' },
  { id: 'tracking', label: 'نظام تتبع', icon: <Users className="w-5 h-5" />, color: 'from-accent to-pink-400' },
];

const industries = [
  'الموارد البشرية', 'المبيعات', 'التعليم', 'المشاريع', 'المخزون', 'المالية', 'التسويق', 'أخرى'
];
  ...
  return (
    ...
    >
      <div className="flex items-center mb-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-text-primary">أنشئ نظامك الآن</h2>
          <p className="text-text-secondary">صف ما تحتاجه بدقة واحصل على نظام كامل</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* نوع القالب */}
        <div>
          <label className="block text-text-primary mb-3 font-medium">اختر نوع النظام</label>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {templateTypes.map((type) => (
              ...
              >
                <div className={`w-10 h-10 rounded-lg bg-gradient-to-br ${type.color} p-2 mb-3`}>
                  {type.icon}
                </div>
        ...
        {/* مجال العمل */}
        <div>
          <label className="block text-text-primary mb-3 font-medium">مجال العمل</label>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <label className="block text-text-primary font-medium">صف نظامك المطلوب</label>
          ...
          <div className="flex items-center justify-between mt-2">
            <div className="flex items-center space-x-4">
              <div className="flex items-center">
                ...
                <span className="text-sm text-text-secondary">اللغة: العربية</span>
              </div>
              <div className="flex items-center">
                <Settings className="w-4 h-4 text-text-secondary mr-2" />
                <span className="text-sm text-text-secondary">ذكاء اصطناعي متقدم</span>
              </div>
            </div>
            <button
              type="button"
              onClick={() => setAdvancedOptions(prev => ({ ...prev, language: prev.language === 'ar' ? 'en' : 'ar' }))}
              className="text-sm text-primary hover:text-primary-light transition-colors duration-300"
            >
              {advancedOptions.language === 'ar' ? 'Switch to English' : 'التبديل للعربية'}
            </button>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="flex items-center space-x-3">
              <input
                ...
                onChange={(e) => setAdvancedOptions(prev => ({ ...prev, includeCharts: e.target.checked }))}
            ...
            <label className="flex items-center space-x-3">
              <input
                ...
                onChange={(e) => setAdvancedOptions(prev => ({ ...prev, notifications: e.target.checked }))}
                className="w-4 h-4 text-primary rounded border-border focus:ring-primary focus:ring-2"
              />
        ...
        <div className="pt-4">
          <button
            ...
            className={`w-full btn-primary py-4 text-lg font-medium ${
              (isGenerating || wordCount < 5) ? 'opacity-50 cursor-not-allowed' : ''
            }`}
          >
            {isGenerating ? (
      ...
      <div className="mt-8 pt-8 border-t border-border">
        <h4 className="font-medium text-text-primary mb-3">نصائح للحصول على أفضل نتيجة:</h4>
